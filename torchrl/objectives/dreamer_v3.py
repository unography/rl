    ...
    *posterior*. Free bits are applied **per categorical** (clamped before
    ...
    Args:
        ...
        unimix (float): Fraction of uniform probability mixed into each
            categorical before the KL. Default: 0.0.
...
class DreamerV3ActorLoss(LossModule):
    ...
    def __init__(
        ...
        *,
        ...
        if imagination_rollout is not None and not use_reinforce:
            raise ValueError("imagination_rollout requires use_reinforce=True.")
    ...
    def set_shared_value_forward(self, value_loss: DreamerV3ValueLoss | None) -> None:
        ...
        if value_loss is None:
            self.__dict__.pop("_shared_value_loss", None)
            return
        ...
        # Held outside the module registry, as continuation_model is: the value
        # loss owns these parameters and must not be duplicated here.
        self.__dict__["_shared_value_loss"] = value_loss
    ...
    def forward(self, tensordict: TensorDict) -> tuple[TensorDict, TensorDict]:
        ...
        shared_value_loss = self.__dict__.get("_shared_value_loss")
        shared_value_forward = (
            None
            if shared_value_loss is None
            else shared_value_loss._shared_value_forward
        )
