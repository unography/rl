...
class _DreamerV3RMSNorm(nn.Module):
    """RMS normalization with a learned scale, as the reference uses."""
...
class _DreamerV3BlockLinear(nn.Module):
    ...
    def forward(self, value: torch.Tensor) -> torch.Tensor:
        ...
        # The reference casts the kernel and bias to the compute dtype, so the
        # bias must not promote a BF16 recurrence back to FP32.
...
class DreamerV3MLP(nn.Module):
    ...
    Args:
        ...
        out_features (int or None): Output feature count. If ``None``, returns
            the last hidden activation without an output projection.
...
class RSSMPriorV3(nn.Module):
    ...
    def _belief_and_logits(
        ...
        """Advance the recurrence and return the unsampled prior logits."""
    ...
    def _update_belief(
        ...
        The reference conditions its filtering policy on the observation right
        after the recurrence, so the acting path never samples the prior.
        """
...
class RSSMRolloutV3(TensorDictModuleBase):
    ...
    The recurrence runs on tensors when the modules carry the standard
    DreamerV3 wiring, fixed at construction, and on TensorDicts otherwise; the
    two agree bit for bit. See :meth:`compile_rollout` to compile it. Entries
    the rollout does not overwrite share storage with the input rather than
    being copied.
        ...
        *batch, time_steps = tensordict.shape
        ...
        for t in range(time_steps):
            ...
            if reset is not None:
                ...
                # The reference masks the incoming action along with the carry,
                # so a reset step cannot condition on the previous action.
                action = (
    ...
    def _forward_fast(self, tensordict):
        """Run the recurrence on tensors and write the TensorDict once."""
    ...
    def _step(self, state, belief, action_t, embedding_t, reset_t):
        """One step of deterministic work.

        Everything here is a function of its inputs, so it can be compiled
        without touching the random stream. The two categorical draws stay
        outside, in :meth:`_scan`.
        """
    ...
    def _scan(self, state, belief, action, embedding, reset):
        ...
        for time_index in range(action.shape[-2]):
            ...
            # The prior sample is overwritten and never read, but drawing it
            # keeps the random stream identical to the TensorDict path.
    ...
    def compile_rollout(
        ...
        ``"step"`` compiles one step of deterministic work and leaves the
        categorical draws in eager, so it draws the same categories and only
        float rounding moves. ``"scan"`` compiles the whole unrolled
        recurrence and is faster, but the draws fall inside the compiled region
        and Inductor does not reproduce eager's random numbers, so a seeded run
        trains a different trajectory.

        Requires the tensor path. ``dynamic=False`` is the default; other
        keyword arguments go to :func:`torch.compile`.
        """
