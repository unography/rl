...
RolloutPath = Literal["tensordict", "tensor", "compiled"]
...
def _call(rollout: RSSMRolloutV3, tensordict: TensorDict, device: torch.device) -> None:
    with torch.inference_mode():
        rollout(tensordict.clone())
    if device.type == "cuda":
        torch.cuda.synchronize()


@pytest.mark.parametrize("path", ["tensordict", "tensor", "compiled"])
def test_dreamer_v3_rssm_rollout(benchmark, path: RolloutPath) -> None:
    """Compare the TensorDict, tensor and compiled RSSM rollouts."""
    ...
    # Compilation and its warmup are excluded from the timed region.
    for _ in range(3 if path == "compiled" else 1):
