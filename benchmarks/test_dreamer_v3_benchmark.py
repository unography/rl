...
"""Benchmarks for the DreamerV3 recurrent state-space model."""
...
def _make_rollout(
    device: torch.device, path: Literal["tensordict", "tensor"]
) -> RSSMRolloutV3:
    torch.manual_seed(0)
    prior = RSSMPriorV3(
    ...
    rollout._fast_path = path == "tensor"
    return rollout
...
# The compiled scopes are not benchmarked here: building one costs minutes,
# well past this suite's 240 s timeout.
@pytest.mark.parametrize("path", ["tensordict", "tensor"])
def test_dreamer_v3_rssm_rollout(
    benchmark, path: Literal["tensordict", "tensor"]
) -> None:
    """Compare the TensorDict and tensor RSSM rollouts."""
