...
def main() -> None:
    ...
    example = runpy.run_path(example_path, run_name="dreamer_v3_latent_fix_repro")
    ...
    (_, encoder, prior, posterior, _, _, _,) = example["build_world_model"](
        cfg=cfg,
    ...
    assert evidence["state_abs_max"] > 0
    assert evidence["belief_abs_max"] > 0
    assert evidence["loc_temporal_range"] > 0
    assert evidence["episode_start_count"] >= 2
    assert evidence["episode_start_belief_abs_max"] == 0
