...
class _AggregateSummary(TypedDict):
    ...
    lower_quartile_return: list[float]
    upper_quartile_return: list[float]
    per_seed_window_median: list[list[float]]
    window_size: int
    seeds: list[int]


def aggregate_runs(paths: list[Path], window_size: int = 50_000) -> _AggregateSummary:
...
def main() -> None:
    ...
    # Hydra changes the training subprocess's working directory. Pass absolute
    # metric paths so the child writes exactly where this parent will read.
