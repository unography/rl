...
pass `env.use_seed=false` for the reference's unseeded DMC resets. The step axis
counts initial and reset-only driver records as JAX does.
...
minimum final median return of 900. Override them for a smaller ablation with
Hydra overrides (`benchmark.window_size=1000`), which both the runs and the
aggregation read. Full learning-curve runs are intended for scheduled or manual
validation; pull-request CI uses short smoke overrides.

`optimization.compile_rssm` compiles the RSSM recurrence, and is off so that a
run reproduces the numbers above. On one GPU, `step` is about 2x on the learner
