...
class _DreamerV3UpdateRatio:
    ...
    uninitialized branch. Rounding each collector batch independently instead
    would drop the remainder and drift whenever ``frames_per_batch *
    train_ratio`` is not a multiple of ``batch_size * seq_len``.
...
class _DreamerV3ReplayRecordBuilder:
    ...
    def __call__(self, data: TensorDictBase) -> TensorDictBase:
        ...
        for time_index in range(data.shape[1]):
            ...
            # The sampler slices along the storage's stream dimension directly,
            # so this is the sampler's declared traj_key rather than a value it
            # reads. Keep it so that configuration is not a dangling reference.
...
class _DreamerV3ReplaySampler(SliceSampler):
    ...
    def sample(self, storage, batch_size: int):
        ...
        # num_slices of them. Admitting fewer would let the queue grow without
        # bound whenever enqueues outpace updates, and the sampler would then
        # serve ever staler starts, which defeats the purpose of online replay.
        # The reference's first, pre-gate prefetch needs no special case: the
        # queue is still empty when it runs.
...
def build_world_model(*, cfg: DictConfig, obs_dim: int, action_dim: int):
    ...
    # One output head per observation event, as the reference's DictHead builds.
    # This is not only structural: the optimizer clips gradients adaptively per
    # parameter, so three head kernels clip differently from one merged kernel
    # covering the same columns.
    #
    # The heads predict in symlog space; symexp back so the loss's
    # symlog(prediction) round-trips to the raw head output, matching the
    # reference's symlog_mse (target squashed, prediction raw). The reference
    # promotes the prediction to FP32 before that comparison, so promote here
    # and keep the round-trip out of BF16.
...
def build_forward_backward(
    *,
    ...
    device: torch.device,
    use_bfloat16: bool,
    continuation_horizon: float,
    lmbda: float,
    replay_value_loss_weight: float,
):
    """Build the fused learner step: one forward, one backward, no optimizer step.

    Returned as a closure so the caller drives one fused step. It returns the
    stacked
    loss metrics plus the detached posterior state and belief, and leaves the
    gradients on the parameters for the caller to consume.
    """
    ...
    def forward_backward(
        ...
        with torch.autocast(
            ...
            post_belief = (
                model_out.get(("next", "belief")).detach().reshape(-1, rnn_hidden_dim)
            )
            ...
            total_loss = (
                ...
                + replay_value_loss_weight * replay_loss
            )
...
def main(cfg: DictConfig):
    ...
    # Windowed loss statistics, independent of the optional end-of-run plot.
    ...
    if update_ratio is not None:
        # Only used for the summary plot title; the loop reads the scheduler.
        updates_per_batch = max(
            1, round(cfg.collector.frames_per_batch * update_ratio.ratio)
        )

    use_bfloat16 = cfg.optimization.mixed_precision and device.type == "cuda"

    forward_backward = build_forward_backward(
    ...
    def train_step(
        ...
        outputs = forward_backward(sample)
        optimizer.step()
    ...
    # One summary record closes the stream.
    _append_jsonl(
        metrics_jsonl_path,
        {
            "type": "summary",
