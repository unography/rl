...
import torch
from dreamer_v3_utils import jax_torch_seed, latent_state_dim, POLICY_RNG_STREAM
...
def _cast_float(key: NestedKey) -> TensorDictModule:
    """Promote one entry to FP32 in place, leaving the rest of the graph BF16."""
...
class DreamerV3SeededPolicy(TensorDictModuleBase):
    ...
    def forward(self, tensordict: TensorDictBase) -> TensorDictBase:
        ...
        with torch.random.fork_rng(devices=devices):
            torch.manual_seed(
                jax_torch_seed(self.seed, self.counter, POLICY_RNG_STREAM)
            )
            self.counter += 1
            return self.module(tensordict)
...
def make_env(cfg: DictConfig, seed: int | None = 0) -> TransformedEnv:
    if cfg.env.backend == "gym":
        ...
        from torchrl.envs.libs.dm_control import (
            DMControlEnv,
        )  # noqa: PLC0415  (optional dep)
...
def make_primed_env(
    ...
    """Environment carrying the recurrent policy's latent and previous action."""
...
def build_world_model(
    *, cfg: DictConfig, obs_dim: int, action_dim: int
    ...
    """MLP encoder + RSSMRolloutV3 + MLP decoder + reward and continuation heads.

    Returns the assembled world model, then the prior, reward, reward-decoder
    and continuation submodules that imagination and the acting policy reuse.
    """
    state_dim = latent_state_dim(cfg)
