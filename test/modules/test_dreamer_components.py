...
class TestDreamerV3Components:
    ...
    def test_rssm_rollout_compiled_loop_matches_eager(self, device, compile_kwargs):
        eager = self._make_rollout(device)
        compiled = copy.deepcopy(eager)
        compiled.compile_rollout("loop", chunk_size=3, fullgraph=True, **compile_kwargs)
        data = self._make_rollout_data(device)

        # Four time steps with chunks of three exercises both the captured
        # full chunk and its shorter tail. Replaying both graphs catches stale
        # cudagraph outputs and backward state that a single call would miss.
        for seed in range(3):
            torch.manual_seed(seed)
            expected = eager(data.clone())
            expected_rng_state = (
            ...
            for actual_gradient, expected_gradient in zip(
                ...
                torch.testing.assert_close(
                    actual_gradient, expected_gradient, atol=5e-4, rtol=2e-4
                )
